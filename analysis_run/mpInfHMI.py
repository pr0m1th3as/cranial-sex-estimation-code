...
for file in train_dir:
    ...
    if (train_HMI.shape[0] == 60) and (train_HMI.shape[1] == 60):
...
for file in test_dir:
    ...
    if (test_HMI.shape[0] == 60) and (test_HMI.shape[1] == 60):
...
for file in cross_dir:
    ...
    if (cross_HMI.shape[0] == 60) and (cross_HMI.shape[1] == 60):
...
os.chdir('./HMI_train')
train_DATA = np.zeros((train_sample_size,60,60))
...
os.chdir('./HMI_test')
test_DATA = np.zeros((test_sample_size,60,60))
...
os.chdir('./HMI_cross')
cross_DATA = np.zeros((cross_sample_size,60,60))

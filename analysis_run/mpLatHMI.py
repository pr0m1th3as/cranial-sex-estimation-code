...
for file in train_dir:
    ...
    if (train_HMI.shape[0] == 80) and (train_HMI.shape[1] == 100):
...
for file in test_dir:
    ...
    if (test_HMI.shape[0] == 80) and (test_HMI.shape[1] == 100):
...
for file in cross_dir:
    ...
    if (cross_HMI.shape[0] == 80) and (cross_HMI.shape[1] == 100):
...
os.chdir('./HMI_train')
train_DATA = np.zeros((train_sample_size,80,100))
...
os.chdir('./HMI_test')
test_DATA = np.zeros((test_sample_size,80,100))
...
os.chdir('./HMI_cross')
cross_DATA = np.zeros((cross_sample_size,80,100))
...
for i in range(0,train_sample_size):
    train_HOGs[i,:] = hog(train_DATA[i],orientations=8,pixels_per_cell=(10,10),cells_per_block=(1,1))
print('HOG training matrix is: ', train_HOGs.shape)
test_HOGs = np.zeros((test_sample_size,n_features))
for i in range(0,test_sample_size):
    test_HOGs[i,:] = hog(test_DATA[i],orientations=8,pixels_per_cell=(10,10),cells_per_block=(1,1))
print('HOG testing matrix is: ', test_HOGs.shape)
cross_HOGs = np.zeros((cross_sample_size,n_features))
for i in range(0,cross_sample_size):
    cross_HOGs[i,:] = hog(cross_DATA[i],orientations=8,pixels_per_cell=(10,10),cells_per_block=(1,1))

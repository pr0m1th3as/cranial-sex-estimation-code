...
% this script utilizes the mpInfHMI.py script to iteratively train and test SVM models, and further
% cross-validate them on an independent sample with the height map images from the inferior view
...
ABH = dir("../HMI/ABH*mastoidRightInferior*");
WLH = dir("../HMI/WLH*mastoidRightInferior*");
MOTOL = dir("../HMI/MOTOL*mastoidRightInferior*");
HOMOL = dir("../HMI/HOMOL*mastoidRightInferior*");
...
for iter = 1:iterations
	...
	for i=1:length(Train_files)
		...
		new_name = ["Train" sprintf("%03d", i) "mrInfHMI.csv"];
		cmd_line = ["cp ../HMI/" old_name " " new_name];
		unix(cmd_line);
	...
	for i=1:length(Test_files)
		...
		new_name = ["Test" sprintf("%03d", i) "mrInfHMI.csv"];
		cmd_line = ["cp ../HMI/" old_name " " new_name];
		unix(cmd_line);
	...
	unix("python3 mpInfHMI.py");
	% delete autogenerated training and testing files from HMI_ folders
	unix("rm ./HMI_train/*");
	unix("rm ./HMI_test/*");
	% read test_classification.csv output file from mpInfHMI.py script
...
csvwrite("mrInf_results.csv", output);
save("mrInf_classification.mat", "classification" );

...
% this script utilizes the mpLatHMI.py script to iteratively train and test SVM models, and further
% cross-validate them on an independent sample with the height map images from the lateral view
% of the right mastoid process area, generated with the skullanalyzer software
clear
iterations = 100;

% create a unified list from all available occipital protuberance HMI files
ABH = dir("../HMI/ABH*mastoidRightLateral*");
WLH = dir("../HMI/WLH*mastoidRightLateral*");
MOTOL = dir("../HMI/MOTOL*mastoidRightLateral*");
HOMOL = dir("../HMI/HOMOL*mastoidRightLateral*");
...
for l=1:length(HOMOL)
	...
	Cross_labels(l,:) = sex(sex(:,1)==sample_id,:);
	Cross_files(l).name = HOMOL(l).name;
end

% define result matrices and resize them appropriately for a given number of iterations
ABH_results = label([1:i],:);
WLH_results = label([i+1:i+j],:);
MOTOL_results = label([i+j+1:i+j+k],:);
HOMOLKA_results = Cross_labels;
...
for iter = 1:iterations
	...
	Train_labels = label(idx([1:151]),:);
	for i=152:t_sample
		Test_files(i-151).name = files(idx(i)).name;
	endfor
	Test_labels = label(idx([152:t_sample]),:);

	% create a copy for each file into the training and testing folders respectively, while
	% renaming each file according the its index on the respective list
	for i=1:length(Train_files)
		old_name = Train_files(i).name;
		new_name = ["Train" sprintf("%03d", i) "mrLatHMI.csv"];
		cmd_line = ["cp ../HMI/" old_name " " new_name];
		unix(cmd_line);
	...
	for i=1:length(Test_files)
		...
		new_name = ["Test" sprintf("%03d", i) "mrLatHMI.csv"];
		cmd_line = ["cp ../HMI/" old_name " " new_name];
		unix(cmd_line);
	...
	unix("python3 mpLatHMI.py");
	% delete autogenerated training and testing files from HMI_ folders
	unix("rm ./HMI_train/*");
	unix("rm ./HMI_test/*");
	% read test_classification.csv output file from mpLatHMI.py script
	...
	% read cross_classification.csv output file from mpLatHMI.py script
	...
	for i=1:cross_samples
		...
		if(f_initials == "HOM")
			f_number = str2num(Cross_files(i).name([6:8]));
...
csvwrite("mrLat_results.csv", output);
save("mrLat_classification.mat", "classification" );

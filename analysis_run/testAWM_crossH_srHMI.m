...
% this script utilizes the srHMI.py script to iteratively train and test SVM models, and further
% cross-validate them on an independent sample with the height map images from the supraorbital
% ridge area, generated with the skullanalyzer software
clear
iterations = 100

% create a unified list from all available occipital protuberance HMI files
ABH = dir("../HMI/ABH*supraorbital*");
WLH = dir("../HMI/WLH*supraorbital*");
MOTOL = dir("../HMI/MOTOL*supraorbital*");
HOMOL = dir("../HMI/HOMOL*supraorbital*");
...
for i = 1:length(Cross_files)
	...
	new_name = ["Cross" sprintf("%03d", i) "srHMI.csv"];
...
for iter = 1:iterations
	...
	for i=1:length(Train_files)
		...
		new_name = ["Train" sprintf("%03d", i) "srHMI.csv"];
		cmd_line = ["cp ../HMI/" old_name " " new_name];
		unix(cmd_line);
	...
	for i=1:length(Test_files)
		...
		new_name = ["Test" sprintf("%03d", i) "srHMI.csv"];
		cmd_line = ["cp ../HMI/" old_name " " new_name];
		unix(cmd_line);
...
csvwrite("sr_results.csv", output);
save("sr_classification.mat", "classification" );
